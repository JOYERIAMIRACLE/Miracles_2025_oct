...
const ROOT: PageNode = { ...blank(""), id: "root", title: "miracles-frontend.pages.dev" }
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 space-y-4">
        ...
        <div className="flex items-center gap-3 px-6 py-4 rounded-xl border border-white/[0.07] bg-[#0d1117]">
          ...
          <span className="text-[14px] font-mono text-slate-500 flex-1 truncate">
            {siteDomain || "miracles-frontend.pages.dev"}{fp !== "/" ? fp : ""}
          </span>
        ...
        {tab === "seo" && (
          <>
              ...
              {node.id === "root" ? (
                <FieldRow label="Dominio">
                  <input value={node.title} onChange={e => onUpdate({ title: e.target.value })}
                    placeholder="ej. miracles-frontend.pages.dev" className={inp} />
                </FieldRow>
              ) : (
                <FieldRow label="Slug">
            ...
            <FieldCard>
              ...
              <FieldRow label="Keywords">
                <input value={node.keywords} onChange={e => onUpdate({ keywords: e.target.value })}
                  placeholder="joyería, anillos de oro, plata 925, Monterrey..." className={inp} />
                {kw.length > 0 && (
            ...
            <FieldCard>
              <FieldRow label="Notas">
                <textarea value={node.notas} onChange={e => onUpdate({ notas: e.target.value })}
                  placeholder="Referencias, decisiones de diseño, Figma links..." rows={3}
                  className={`${inp} resize-none`} />
              </FieldRow>
            </FieldCard>
            ...
            <button
              type="button"
              onClick={() => {
                const domain = siteDomain || "miracles-frontend.pages.dev"
...
export function SitioWebMiraclesView() {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE}/api/sitio-web-miracles`, {
  ...
  const exportJson = () => {
    ...
    const url  = URL.createObjectURL(blob)
    const a    = Object.assign(document.createElement("a"), { href: url, download: "sitio-web-miracles.json" })
    a.click()
    URL.revokeObjectURL(url)
  }
    ...
    <div
      ...
      {modalNode && modal && (
        <DrawerPagina
          ...
          siteDomain={find(tree, "root")?.title ?? "miracles-frontend.pages.dev"}
