...
function MesGroup({ mes, anio, campanas, onEdit, onDelete, onNueva }: {
  ...
  const objetivoKey = `campana-empresa-objetivo-${mes}-${anio}`
      ...
      {!collapsed && (
        <>
          <div className="px-4 py-2 border-b border-slate-800/50 flex items-center gap-2 min-h-[36px]">
            {editingObjetivo ? (
              <input autoFocus value={objetivoDraft}
              ...
              return (
                <div key={n} className="flex items-center gap-3 px-4 py-3 min-h-[52px]">
                  ...
                  <div className="flex-1 flex flex-wrap items-center gap-2">
                    {items.map(({ c, titulo }) => (
                      <CampanaChip key={`${c.documentId}-${n}`} titulo={titulo} categoria={c.categoria}
  ...
  return (
    <div className="space-y-3">
      {grupos.map(({ mes, anio, items }) => (
        <MesGroup key={`${mes}-${anio}`} mes={mes} anio={anio} campanas={items}
          onEdit={onEdit} onDelete={onDelete}
          onNueva={({ mes: m, anio: a }) => { onNueva() }}
        />
      ))}
    </div>
  )
                ...
                return (
                  <div key={di}
                    ...
                    {items.map(({ campana, n, titulo }) => (
                      <CampanaChip key={`${campana.documentId}-${n}`} titulo={titulo} categoria={campana.categoria}
