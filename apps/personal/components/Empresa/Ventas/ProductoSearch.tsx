...
export function ProductoSearch({
  ...
  return (
    <div className="relative">
      ...
      />
      {open && (
        <div className="absolute z-20 top-full left-0 w-80 mt-0.5 bg-slate-900 border border-slate-700 rounded-lg shadow-2xl overflow-hidden">
          <div className="flex flex-wrap gap-1 p-2 border-b border-slate-800">
            <button type="button" onMouseDown={e => e.preventDefault()} onClick={() => setFiltroCat("todas")}
              className={`h-6 px-2 rounded-full text-[10px] font-medium border transition-all ${filtroCat==="todas"?"bg-slate-700 text-slate-100 border-slate-600":"border-slate-700 text-slate-500 hover:text-slate-300"}`}>
              Todas
            </button>
            {CATEGORIAS_JOYA.map(c => (
              <button key={c} type="button" onMouseDown={e => e.preventDefault()}
                onClick={() => setFiltroCat(prev => prev===c?"todas":c)}
                className={`h-6 px-2 rounded-full text-[10px] font-medium border transition-all ${filtroCat===c?`${CAT_COLOR[c]} shadow-sm`:"border-slate-700 text-slate-500 hover:text-slate-300"}`}>
                {c}
              </button>
            ))}
          </div>
          <div className="max-h-64 overflow-y-auto">
            {filtered.length === 0 && (
              <p className="px-3 py-4 text-[11px] text-slate-600 text-center">Sin productos para este filtro.</p>
            )}
            {filtered.map(p => (
              <button key={p.documentId} type="button"
                onMouseDown={e => { e.preventDefault(); onSelect(p); setOpen(false); setFiltroCat("todas") }}
                className="w-full flex items-center justify-between px-3 py-2 hover:bg-slate-700/80 transition text-left gap-2 border-b border-slate-800/60 last:border-0">
                ...
                <div className="text-right shrink-0">
                  ...
                  <span className="text-[9px] text-slate-600">stock {p.stock ?? 0}</span>
                </div>
              </button>
            ))}
          </div>
