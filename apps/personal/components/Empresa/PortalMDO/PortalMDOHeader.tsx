...
function LogoEditable({ url, documentId, onUploaded }: { url?: string | null; documentId: string | null; onUploaded: () => void }) {
  ...
  return (
    <div className={`relative h-9 w-9 shrink-0 flex items-center justify-center overflow-hidden rounded-lg group ${url ? "bg-[#2a1b3d] p-1" : "bg-violet-600"}`}>
...
function ThemeToggleButton() {
  ...
  return (
    <button type="button" aria-label="Cambiar tema"
      ...
      className="h-9 w-9 rounded-lg flex items-center justify-center text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-100 hover:bg-slate-100 dark:hover:bg-[#2a1b3d] transition-colors">
  ...
  return (
    <div ref={ref} className="relative">
      <button type="button" aria-label="Notificaciones" onClick={() => setOpen(o => !o)}
        className={`relative h-9 w-9 rounded-lg flex items-center justify-center transition-colors ${open ? "bg-violet-50 dark:bg-violet-950/30 text-violet-500" : "text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-100 hover:bg-slate-100 dark:hover:bg-[#2a1b3d]"}`}>
        <Bell className="h-5 w-5" />
      </button>
      {open && (
        <div className="fixed left-4 right-4 top-16 sm:absolute sm:left-auto sm:right-0 sm:top-full sm:mt-2 sm:w-80 bg-white dark:bg-[#2a1b3d] rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 z-200 overflow-hidden">
...
export function PortalMDOHeader({ onMenuClick, onLogoClick, onNavigate }: Props) {
  ...
  return (
    <>
      <header className="h-14 bg-[#f8f9fa] dark:bg-[#2a1b3d] border-b border-slate-300 dark:border-slate-700 shadow-sm flex items-center gap-2 sm:gap-3 px-2 sm:px-4 sticky top-0 z-50">
        <button type="button" title="Menú" onClick={onMenuClick}
          className="h-9 w-9 rounded flex items-center justify-center text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-100 hover:bg-slate-100 dark:hover:bg-[#2a1b3d] transition-colors shrink-0">
        ...
        <div className="ml-auto flex items-center gap-1.5 sm:gap-2">
          <button type="button" aria-label="Buscar"
            ...
            className={`sm:hidden h-8 w-8 rounded flex items-center justify-center transition-colors ${mobileSearchOpen ? "bg-violet-50 dark:bg-violet-950/30 text-violet-500" : "text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-[#2a1b3d]"}`}>
      ...
      {mobileSearchOpen && (
        <div className="sm:hidden sticky top-14 z-50 bg-white dark:bg-[#2a1b3d] border-b border-slate-300 dark:border-slate-700 px-4 py-2.5">
