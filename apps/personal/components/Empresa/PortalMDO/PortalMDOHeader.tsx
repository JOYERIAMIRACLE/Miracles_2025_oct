...
function SearchResultsPanel({ results, onSelect }: { results: Resultado[]; onSelect: (r: Resultado) => void }) {
  if (results.length === 0) {
    return (
      <div className="absolute left-0 right-0 top-full mt-2 bg-white dark:bg-[#2a1b3d] rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 z-[200] p-4">
        <p className="text-xs text-slate-400 text-center">Sin resultados</p>
      </div>
    )
  }
  return (
    <div className="absolute left-0 right-0 top-full mt-2 bg-white dark:bg-[#2a1b3d] rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 z-[200] overflow-hidden max-h-72 overflow-y-auto">
      {results.map(r => (
        <button key={`${r.seccion}-${r.tab ?? ""}`} type="button" onClick={() => onSelect(r)}
          className="w-full text-left px-4 py-2.5 hover:bg-slate-50 dark:hover:bg-[#2a1b3d] transition-colors">
...
function LogoEditable({ url, documentId, onUploaded }: { url?: string | null; documentId: string | null; onUploaded: () => void }) {
  ...
  return (
    <div className={`relative h-10 w-10 shrink-0 flex items-center justify-center overflow-hidden rounded-lg group ${url ? "bg-[#2a1b3d] p-1" : "bg-violet-600"}`}>
...
function ThemeToggleButton() {
  ...
  return (
    <button type="button" aria-label="Cambiar tema"
      ...
      className="h-10 w-10 rounded-lg flex items-center justify-center text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-100 hover:bg-slate-100 dark:hover:bg-[#2a1b3d] transition-colors">
  ...
  return (
    <div ref={ref} className="relative">
      <button type="button" onClick={() => setOpen(o => !o)} aria-label="Perfil"
        className="h-10 w-10 rounded-full overflow-hidden shrink-0 ring-2 ring-transparent hover:ring-violet-400 transition-all text-[11px]">
        <AvatarCircle ini={ini} fotoUrl={user?.fotoUrl} />
      </button>
      {open && (
        <div className="absolute right-0 top-full mt-2 w-52 bg-white dark:bg-[#2a1b3d] rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 z-200 overflow-hidden">
  ...
  return (
    <div ref={ref} className="relative">
      <button type="button" aria-label="Notificaciones" onClick={() => setOpen(o => !o)}
        className={`relative h-10 w-10 rounded-lg flex items-center justify-center transition-colors ${open ? "bg-violet-50 dark:bg-violet-950/30 text-violet-500" : "text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-100 hover:bg-slate-100 dark:hover:bg-[#2a1b3d]"}`}>
        <Bell className="h-5.5 w-5.5" />
      </button>
      {open && (
        <div className="fixed left-4 right-4 top-[72px] sm:absolute sm:left-auto sm:right-0 sm:top-full sm:mt-2 sm:w-80 bg-white dark:bg-[#2a1b3d] rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 z-200 overflow-hidden">
...
export function PortalMDOHeader({ onMenuClick, onLogoClick, onNavigate }: Props) {
  ...
  return (
    <>
      <header className="h-16 bg-[#f8f9fa] dark:bg-[#2a1b3d] border-b border-slate-300 dark:border-slate-700 shadow-sm flex items-center gap-2.5 sm:gap-4 px-3 sm:px-5 sticky top-0 z-50">
        <button type="button" title="Menú" onClick={onMenuClick}
          className="h-10 w-10 rounded-lg flex items-center justify-center text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-100 hover:bg-slate-100 dark:hover:bg-[#2a1b3d] transition-colors shrink-0">
        ...
        <div role="button" tabIndex={0} onClick={onLogoClick}
          ...
          <div className="min-w-0 text-left hidden sm:block">
            <div className="text-lg font-bold text-slate-900 dark:text-slate-100 leading-tight">Portal organizacional</div>
            <div className="text-xs text-slate-400 leading-none pt-0.5">Medalla de Oro</div>
          </div>
        </div>

        <div ref={searchRef} className="hidden sm:flex flex-1 justify-center px-8 relative">
          <div className="relative w-80">
            <div className="flex items-center gap-2 bg-slate-100 dark:bg-[#2a1b3d] rounded-lg px-3 py-1.5">
              ...
              {busqueda && (
                <button type="button" onClick={() => { setBusqueda(""); setSearchOpen(false) }}>
                  <XIcon size={13} className="text-slate-400 hover:text-slate-600" />
                </button>
              )}
            </div>
        ...
        <div className="ml-auto flex items-center gap-2 sm:gap-2.5">
          <button type="button" aria-label="Buscar"
            ...
            className={`sm:hidden h-9 w-9 rounded-lg flex items-center justify-center transition-colors ${mobileSearchOpen ? "bg-violet-50 dark:bg-violet-950/30 text-violet-500" : "text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-[#2a1b3d]"}`}>
      ...
      {mobileSearchOpen && (
        <div className="sm:hidden sticky top-16 z-50 bg-white dark:bg-[#2a1b3d] border-b border-slate-300 dark:border-slate-700 px-4 py-2.5">
          <div className="relative">
            <div className="flex items-center gap-2 bg-slate-100 dark:bg-[#2a1b3d] rounded-lg px-3 py-2">
