...
function ModalEnlace({ mode, onSave, onCancel }: {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={cerrarSiVacio}>
      <div className="bg-white dark:bg-[#2a1b3d] border border-slate-200 dark:border-slate-700 shadow-md rounded-xl w-full max-w-md p-6 space-y-3 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between pb-1">
          ...
          <button type="button" title="Cerrar" onClick={onCancel}
            className="p-1.5 text-slate-500 hover:text-slate-700 dark:hover:text-slate-200 rounded hover:bg-slate-100 dark:hover:bg-[#2a1b3d] transition">
            <X size={16} />
          </button>
        </div>
...
function EnlaceCard({ m, onEdit, onDelete }: {
  ...
  return (
    <div
      ...
      className="group flex items-center gap-3 p-3 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-[#2a1b3d] shadow-sm hover:border-slate-400 dark:hover:border-slate-600 hover:shadow-md transition-all cursor-pointer"
    >
      <div className="h-10 w-10 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-[#2a1b3d] flex items-center justify-center shrink-0">
          ...
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div className="relative w-full max-w-sm" ref={searchRef}>
              ...
              <input value={busqueda} onChange={e => setBusqueda(e.target.value)} onFocus={() => setDropdownOpen(true)}
                ...
                className="w-full pl-8 pr-8 h-9 text-sm rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-[#2a1b3d] text-slate-700 dark:text-slate-200 placeholder:text-slate-400 outline-none focus:border-violet-400 transition shadow-sm" />
              ...
              {dropdownOpen && (
                <div className="absolute top-full left-0 right-0 mt-1.5 bg-white dark:bg-[#2a1b3d] border border-slate-200 dark:border-slate-700 shadow-xl rounded-xl z-30 max-h-72 overflow-y-auto py-1">
                  {filtrados.length === 0 ? (
                    ...
                    filtrados.map(m => (
                      <button key={m.documentId} type="button" onClick={() => abrirEnlace(m)}
                        className="w-full flex items-center gap-2.5 text-left px-3 py-2 transition-colors hover:bg-slate-50 dark:hover:bg-[#2a1b3d]">
