...
export function EmpresaHeader({ onMenuClick }: { onMenuClick?: () => void }) {
  ...
  return (
    <header className="h-14 flex items-center justify-between border-b border-slate-800/80 bg-[#2a1b3d]/95 backdrop-blur supports-backdrop-filter:bg-[#2a1b3d]/80 px-4 md:px-6 sticky top-0 z-40">
      <div className="flex items-center gap-3">
        <button
          type="button"
          onClick={onMenuClick}
          className="md:hidden p-1.5 rounded-lg text-slate-500 hover:text-slate-300 hover:bg-[#2a1b3d] transition-colors"
          title="Menú"
        >
          <Menu size={20} />
        </button>
      ...
      <div className="flex items-center gap-2">
        <Link
          href="/empresa-rpg"
          className="p-1.5 text-slate-600 hover:text-violet-400 hover:bg-[#2a1b3d] rounded-lg transition-colors"
          title="Modo Juego"
        >
          <Gamepad2 size={15} />
        </Link>
        ...
        <button type="button" onClick={handleLogout} title="Cerrar sesión"
          className="p-1.5 text-slate-600 hover:text-slate-300 hover:bg-[#2a1b3d] rounded-lg transition-colors">
          <LogOut size={15} />
        </button>
