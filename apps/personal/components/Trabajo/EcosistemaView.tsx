        ...
        <div key={s.etapa}>
          {i > 0 && (
            <div className="flex flex-col items-center py-1 gap-0.5">
              {s.prev && <span className="text-[10px] font-semibold text-slate-500">{s.prev}</span>}
          ...
          <div className={`flex items-center gap-4 px-5 py-3 rounded-xl border ${s.border} ${s.bg}`}>
            ...
            {s.value !== null
              ? <p className="text-base font-bold text-slate-100 shrink-0">{s.value}</p>
              : <p className="text-[11px] text-slate-500 italic shrink-0">Cualitativo</p>
            }
          </div>
        </div>
      ))}
