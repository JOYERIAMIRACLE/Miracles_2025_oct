...
import { ESTADO_COT_TIENDA, fmtDtCorta } from "../../cuenta-shared"
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {cotizaciones.map(c => (
            <div key={c.documentId} className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl p-5">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <span className="text-sm font-semibold text-gray-900 dark:text-gray-100">{formatPrice(c.total)}</span>
                </div>
              </div>
                ...
                <div className="mt-3 pt-3 border-t border-gray-100 dark:border-zinc-800 space-y-1.5">
                  {c.items.map((item, i) => (
                    <div key={i} className="flex items-center justify-between text-sm gap-3">
                      ...
                      <p className="text-gray-500 dark:text-gray-400 shrink-0">×{item.cantidad} · {formatPrice(item.subtotal)}</p>
                    </div>
                  ))}
                </div>
              )}
